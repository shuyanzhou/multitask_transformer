...
from . import (
    ...
    FairseqModel, register_model, register_model_architecture, BaseFairseqModel
)
...
class MultitaskTransformerModel(BaseFairseqModel):
    ...
    def __init__(self, encoder, decoder_clean, decoder_translation):
        super().__init__()
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        return x, {'attn': attn, 'inner_states': inner_states, 'before_linear': before_linear}
...
class TransformerTranslationDecoder(FairseqIncrementalDecoder):
    """
    Transformer decoder consisting of *args.decoder_layers* layers. Each layer
    is a :class:`TransformerDecoderLayer`.
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, final_norm=True):
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            TransformerTranslationDecoderLayer(args, no_encoder_attn)
            for _ in range(args.decoder_layers)
        ])
    ...
    def forward(self, prev_output_tokens, encoder_out=None, decoder_out=None, incremental_state=None):
        """
        Args:
            prev_output_tokens (LongTensor): previous decoder outputs of shape
                `(batch, tgt_len)`, for input feeding/teacher forcing
        ...
        for layer in self.layers:
            x, encoder_attn, decoder_attn = layer(
                ...
                encoder_out['encoder_out'] if encoder_out is not None else None,
                encoder_out['encoder_padding_mask'] if encoder_out is not None else None,
                decoder_out['decoder_out'] if decoder_out is not None else None,
                decoder_out['decoder_padding_mask'] if decoder_out is not None else None,
                incremental_state,
                self_attn_mask=self.buffered_future_mask(x) if incremental_state is None else None,
            )
            inner_states.append(x)
...
class TransformerTranslationDecoderLayer(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, decoder_out, decoder_padding_mask, incremental_state,
                prev_self_attn_state=None, prev_attn_state=None, self_attn_mask=None,
                self_attn_padding_mask=None):
        """
        Args:
        ...
        return x, attn_encoder, attn_decoder
